...
namespace Aztech\Events\Tests\Core\Transport\Socket
{

    use Aztech\Events\Core\Transport\Socket\Wrapper;
use Symfony\Component\Console\Logger\ConsoleLogger;
use Symfony\Component\Console\Output\ConsoleOutput;
use Aztech\Events\Core\Transport\Socket\SocketBehavior;
    ...
    class SocketWrapperTest extends \PHPUnit_Framework_TestCase
    {
        ...
        protected function setUp()
        {
            ...
            $sock = \Aztech\Events\Core\Transport\Socket\socket_create(AF_INET, SOCK_STREAM);

            // Bind the socket to an address/port
            if (! \Aztech\Events\Core\Transport\Socket\socket_bind($sock, 'localhost', 0)) {
                throw new \RuntimeException('Could not bind to address');
            }

            // Start listening for connections
            \Aztech\Events\Core\Transport\Socket\socket_listen($sock);

            // Accept incoming requests and handle them as child processes.
            $this->client = \Aztech\Events\Core\Transport\Socket\socket_accept($sock);
            $this->server = $sock;
